...
module.exports = {
  buildResponse(ParentResponse) {
    function Response(res, request, routeContext) {
      this.res = res
...
const ResponsePrototype = {
  ...
  set(field, value) {
    ...
    for (var i = 0; i < fields.length; i++) {
      this.res.setHeader(fields[i], field[fields[i]])
    }
    return this
  },

  append(field, value) {
    const curVal = this.res.getHeader(field)
  ...
  send(payload) {
    ...
    if (contentType === undefined) {
      if (typeof payload === 'string') {
        ...
        payload = serialize(this.route, payload, this.res.statusCode)
      }
    } else if (
