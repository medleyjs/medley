...
function routeHandler(req, res, params, context) {
  ...
  var reply = new context.Reply(res, request, context, context.config)

  if (context.onResponse !== null) {
    var onResponseContext = {reply, listener: null}
    var listener = runOnResponseHooks.bind(onResponseContext)
    onResponseContext.listener = listener
...
function runOnResponseHooks() {
  ...
  const onResponseHooks = reply._context.onResponse
  ...
  for (var i = 0; i < onResponseHooks.length; i++) {
    onResponseHooks[i](reply._request, reply)
  }
}
