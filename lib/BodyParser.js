...
BodyParser.prototype.add = function(contentType, opts, parserFn) {
  if (typeof contentType !== 'string') {
    throw new Error('The content type should be a string')
  }
  if (contentType.length === 0) {
    throw new Error('The content type cannot be an empty string')
  }
  if (typeof parserFn !== 'function') {
    throw new Error('The content type handler should be a function')
  }
...
function buildBodyParser(c) {
  const bodyParser = new BodyParser()
  Object.assign(bodyParser.customParsers, c.customParsers)
  bodyParser.parserList = c.parserList.slice()
  return bodyParser
}
...
module.exports.buildBodyParser = buildBodyParser
