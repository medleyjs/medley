...
BodyParser.prototype.run = function(
  ...
  if (parser === undefined) {
    response.code(415).error(new Error('Unsupported Media Type: ' + contentType))
    return
  }

  if (parser.asString === true || parser.asBuffer === true) {
    var {parserOptions} = response._context
    var bodyLimit = parserOptions.limit === null ? parser.bodyLimit : parserOptions.limit

    if (contentLength > bodyLimit) {
      response.code(413).error(new Error('Request body is too large'))
      return
    }
...
function rawBody(req, response, parser, contentLength, bodyLimit, done) {
  ...
  function onEnd(err) {
    ...
    if (contentLength !== -1 && receivedLength !== contentLength) {
      response.code(400).error(new Error('Request body size did not match Content-Length'))
      return
    }
