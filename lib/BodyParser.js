...
BodyParser.prototype.run = function(
  ...
  if (parser === undefined) {
    reply.code(415).error(new Error('Unsupported Media Type: ' + contentType))
    return
  }

  if (parser.asString === true || parser.asBuffer === true) {
    var {parserOptions} = reply._context
    var bodyLimit = parserOptions.limit === null ? parser.bodyLimit : parserOptions.limit

    if (contentLength > bodyLimit) {
      reply.code(413).error(new Error('Request body is too large'))
      return
    }

    rawBody(request.req, reply, parser, contentLength, bodyLimit, done)
  } else {
  ...
  function done(error, body) {
    if (error) {
      ...
      request.body = body
      runPreHandlerHooks(reply, reply._context)
    }
  }
}

function rawBody(req, reply, parser, contentLength, bodyLimit, done) {
  var {asString} = parser
  ...
  function onEnd(err) {
    ...
    if (contentLength !== -1 && receivedLength !== contentLength) {
      reply.code(400).error(new Error('Request body size did not match Content-Length'))
      return
    }
